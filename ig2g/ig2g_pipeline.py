...
# from your_custom_datamanager import InstructGaussian2GaussianDataManagerConfig
from ig2g.ip2p import InstructPix2Pix


@dataclass
class InstructGaussian2GaussianPipelineConfig:
    """Configuration for pipeline instantiation"""

    _target: Type = field(default_factory=lambda: InstructGaussian2GaussianPipeline)
    """target class to instantiate"""
    # datamanager: InstructGaussian2GaussianDataManagerConfig = InstructGaussian2GaussianDataManagerConfig()
    ...
    edit_rate: int = 10
    """how many steps before image edit"""
    edit_count: int = 1
    """how many images to edit per step"""
...
class InstructGaussian2GaussianPipeline:
    """InstructGaussian2Gaussian pipeline"""

    config: InstructGaussian2GaussianPipelineConfig
    ...
    def __init__(
        ...
        self.text_embedding = self.ip2p.pipe._encode_prompt(
            self.config.prompt, device=self.ip2p_device, num_images_per_prompt=1, do_classifier_free_guidance=True,
            negative_prompt=""
        )

        # keep track of spot in dataset
        if self.config.datamanager.train_num_images_to_sample_from == -1:
            self.train_indices_order = cycle(
                range(len(self.config.datamanager.train_dataparser_outputs.image_filenames)))
        else:
            self.train_indices_order = cycle(range(self.config.datamanager.train_num_images_to_sample_from))

        # # viewer elements
        # self.prompt_box = ViewerText(name="Prompt", default_value=self.config.prompt, cb_hook=self.prompt_callback)
        # self.guidance_scale_box = ViewerNumber(name="Text Guidance Scale", default_value=self.config.guidance_scale,
        #                                        cb_hook=self.guidance_scale_callback)
        # self.image_guidance_scale_box = ViewerNumber(name="Image Guidance Scale",
